/**
 * courseCommands.test.js
 * -----------------------------------------
 * Tests for course CRUD operations and JOIN/LEAVE logic.
 */
...
// Absolute paths for consistent behavior
...
describe('Course Commands', () => {
  ...
  test('GETALL should return all courses', () => {
    ...
    const courses = handleCourseCommand('GETALL', []);
  ...
  test('JOIN should not allow a trainee to join more than 5 courses', () => {
    fs.writeFileSync(
      TRAINEE_DATA_FILE_PATH,
      JSON.stringify(
        [{ id: 50, firstName: 'Limit', lastName: 'Tester' }],
        null,
        2
      )
    );
    ...
    for (let i = 1; i <= 6; i++) {
      handleCourseCommand('ADD', [`Course${i}`, `2025-0${i}-01`]);
    }

    let courses = loadCourseData();

    for (let i = 0; i < 5; i++) {
      handleCourseCommand('JOIN', [String(courses[i].id), '50']);
    }

    const sixthCourseId = courses[5].id;
    handleCourseCommand('JOIN', [String(sixthCourseId), '50']);

    courses = loadCourseData();

    const joinedCount = courses.filter((c) =>
      c.participants.includes(50)
    ).length;

    expect(joinedCount).toBe(5);
  });
});
